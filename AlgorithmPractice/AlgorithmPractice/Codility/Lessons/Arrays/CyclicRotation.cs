...
namespace AlgorithmPractice.Codility.Lessons.Arrays
{
    public class CyclicRotation
    {
        ...
        public static int[] Solution(int[] A, int K)
        {
            ...
            var shiftTimes = K % A.Length;
            if (shiftTimes == 0) return A;
            var result = new int[A.Length];
            Array.Copy(A, A.Length - shiftTimes, result, 0, shiftTimes);
            Array.Copy(A, 0, result, shiftTimes, A.Length - shiftTimes);
