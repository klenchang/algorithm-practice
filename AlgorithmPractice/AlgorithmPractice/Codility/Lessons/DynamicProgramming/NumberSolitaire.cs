namespace AlgorithmPractice.Codility.Lessons.DynamicProgramming
{
    public class NumberSolitaire
    {
        ...
        At the beginning of the game, there is a pebble on square number 0 and 
        this is the only square on the board which is marked. 
        ...
        If square number I + K does not exist, we throw the die again until we obtain 
        ...
        {
            var maxSums = new int[A.Length];
            maxSums[0] = A[0];
            for (int x = 1; x < maxSums.Length; x++)
            {
                var startIndex = x >= 6 ? x - 6 : 0;
                var max = maxSums[startIndex];
                for (int y = startIndex + 1; y < x; y++)
                {
                    if (maxSums[y] > max)
                        max = maxSums[y];
                }
