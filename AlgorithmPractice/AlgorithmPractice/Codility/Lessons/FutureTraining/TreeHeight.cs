...
namespace AlgorithmPractice.Codility.Lessons.FutureTraining
{
    public class TreeHeight
    {
        ...
        A binary tree is either an empty tree or a node (called the root) consisting of 
        a single integer value and two further binary trees, called the left subtree and 
        the right subtree.
        ...
        A path in a binary tree is a non-empty sequence of nodes that one can traverse by 
        following the pointers. 
        The length of a path is the number of pointers it traverses. 
        More formally, a path of length K is a sequence of nodes P[0], P[1], ..., P[K], 
        such that node P[I + 1] is the root of the left or right subtree of P[I], 
        for 0 ≤ I < K. 
        For example, the sequence of nodes with values 5, 3, 21 is a path of length 2 
        in the tree from the above figure. 
        ...
        In particular, a tree consisting of only one node has height 0 and, conventionally, 
        an empty tree has height −1. For example, the tree shown in the above figure is of height 2.
        ...
        as explained above. Note that the values contained in the nodes are not relevant in this task.
        ...
        For the purpose of entering your own test cases, you can denote a tree recursively 
        in the following way. An empty binary tree is denoted by None. 
        A non-empty tree is denoted as (X, L, R), where X is the value contained in the root 
        and L and R denote the left and right subtrees, respectively. 
        ...
        {
            return FindDepth(T, 0);
        }

        public static int FindDepth(Tree T, int depth)
        {
            var leftDepth = depth;
            if (T.l != null)
                leftDepth = FindDepth(T.l, leftDepth + 1);
            var rightDepth = depth;
            if (T.r != null)
                rightDepth = FindDepth(T.r, rightDepth + 1);
