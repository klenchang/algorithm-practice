...
namespace AlgorithmPractice.Codility.Lessons.MaximumSliceProblem
{
    public class MaxDoubleSliceSum
    {
        ...
        /// <summary>
        /// time complexity: O(N)
        /// use Y as middle point to caculate max value from X -> Y and Z -> Y
        /// and find out max sum of left + right
        /// </summary>
        /// <param name="A"></param>
        /// <returns></returns>
        public static int Solution(int[] A)
        {
            ...
            var maxLeft = new int[A.Length];
            var maxRight = new int[A.Length];
            ...
            {
                var sum = maxLeft[i - 1] + maxRight[i + 1];
                if (sum > max) max = sum;
            }
