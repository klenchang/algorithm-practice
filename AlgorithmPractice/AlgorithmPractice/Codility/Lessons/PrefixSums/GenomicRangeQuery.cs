...
namespace AlgorithmPractice.Codility.Lessons.PrefixSums
{
    public class GenomicRangeQuery
    {
        ...
        What is the minimal impact factor of nucleotides contained in 
        a particular part of the given DNA sequence?
        ...
        public static int[] Solution(string S, int[] P, int[] Q)
        {
            ...
            for (int index = 0; index < result.Length; index++)
                result[index] = mapping[S.Substring(P[index], Q[index] - P[index] + 1).Min()];
        ...
        {
            var rowMapping = new Dictionary<char, byte>
            {
                { 'A', 0 },
                { 'C', 1 },
                { 'G', 2 },
            };
            var prefixSum = new uint[3, S.Length + 1];
            for (int index = 0; index < S.Length; index++)
            {
                for (int row = 0; row < 3; row++)
                    prefixSum[row, index + 1] = prefixSum[row, index];

                if (rowMapping.ContainsKey(S[index]))
                    prefixSum[rowMapping[S[index]], index + 1]++;
            }
            var result = new int[P.Length];
